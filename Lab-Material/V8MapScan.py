...
class v8_instancetypeaddr(common.AbstractWindowsCommand):
    ...
    def calculate(self):
		...
		#print(str(task.UniqueProcessId))
                continue
            else:
                print("Scanning {0} pid: {1}".format(task.ImageFileName, task.UniqueProcessId))
                ...
                print("Number   Object Address   Instance Type   Data")
                count = 1
                for obj in objs:
                    print(str(count) + "        " + str(hex(obj.address)) + "        " +str(hex(obj.map.instance_type)) + "        " + str(obj.data))
                    count = count + 1
                exit()
